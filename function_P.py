...
def tab_bin(nn,ll):
	...
	tab = [[ int(tab_fact[x+y-1]/(tab_fact[x]*tab_fact[y-1])) for y in range(ll+1)] for x in range(nn+1)]
...
def Hop_prep(X,Y):
	...
	return [TO_con_0(2**i+2**((i+1)%(X+Y-1)),X+Y-1) for i in range(Hop_dim)]
...
def bose_Hamiltonian (BC,t,U,DIM_H):
...
##----- KINETIC
		...
		if BC == 0:
			if state[0] == '1':
				...
				j = get_index(PBC_newstate1,DIM_H)	
		...
		#---- KINETIC = we store i,j,kin_val !!!!
			...
			if state[-1] == '1':
				...
				j = get_index(PBC_newstate2,DIM_H)	
...
def bose_Hamiltonian_1(BC,t,U):

#list(itertools.chain.from_iterable(l))
	
	ham_tmp = [ evaluate_ham(i,BC,t,U) for i in range(DIM_H)]
	ham = list(itertools.chain.from_iterable(ham_tmp))
...
def bose_Hamiltonian_parity_0(base_parity_ind,BC,t,U,parity):
